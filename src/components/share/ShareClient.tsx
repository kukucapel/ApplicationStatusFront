    ...
    <>
      ...
      <div className="mt-6 space-y-10">
        <ShareSectionCard
          title="Ваш вопрос"
          subtitle={
            <span>Последнее обновление: {formatDate(request.updated_at)}</span>
          }
        >
        ...
        {modalInvite && (
          <Modal title={'Детали ответа'} onClose={() => setModalInvite(null)}>
            <ModalBodyResponse
              ...
              {responses.slice().map((r, index) => (
                ...
                >
                  <div
                    ...
                    onClick={() => setModalInvite([index, r])}
                  >
                    <div className="text-sm flex text-[--ink-700] gap-0.5 mb-1">
                      ...
                      >
                        {r.type === 'invite_yes'
                          ? '· Приглашение принято'
                          : r.type === 'invite_no' && '· Приглашение отклонено'}
                      </span>
                      <span className="hidden md:block text-orange-700 font-medium">
                        {r.type === 'invite' && '· Приглашение'}
                      </span>
                    </div>
                  ...
                  <div className="hidden md:block">
                    {r.rating === null ? (
                      <Raiting
                        ...
                        handleSetRating={handleSetRating}
                        handleSubmit={() => {
                          handleSubmitRating(r.id, rating[r.id], index);
                        }}
                        rating={rating}
                        id={r.id}
                      />
                    ) : (
                      <Raiting type="view" responseRating={r.rating} />
                    )}
                  </div>
                </li>
              ))}
        ...
        <ShareSectionCard title="История статусов">
          <Timeline>
            ...
            {history.length ? (
              history
                .slice()
                .filter((h) => h.old_status !== h.new_status && h.old_status)
                .map((h) => (
