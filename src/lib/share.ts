...
export async function getShareApplication(token: string) {
  return await apiRequest(`/user/share/${token}`, {
    cache: 'no-store',
  });
}

export async function setShareRating(id: number, score: number, token: string) {
  return await apiRequest(`/user/share/${token}/responses/${id}/rating`, {
    method: 'POST',
    headers: {
...
export async function setShareInviteChoise(
  ...
  return await apiRequest(`/user/share/${token}/responses/${id}/choice`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
    },
